...
object PlaylistsHelper {
    ...
    suspend fun createPlaylist(playlistName: String) = playlistsRepository.createPlaylist(playlistName)
    ...
    suspend fun importPlaylists(playlists: List<PipedImportPlaylist>) = playlistsRepository.importPlaylists(playlists)
    ...
    fun getPrivatePlaylistType(): PlaylistType = if (loggedIn) PlaylistType.PRIVATE else PlaylistType.LOCAL

    fun getPlaylistType(playlistId: String): PlaylistType =
        if (playlistId.isDigitsOnly()) {
            PlaylistType.LOCAL
