...
private fun VideoStream.toPipedStream() =
    PipedStream(
        url = content,
        codec = codec,
...
private fun AudioStream.toPipedStream() =
    PipedStream(
        url = content,
        format = format?.toString(),
...
fun InfoItem.toContentItem() =
    when (this) {
        is StreamInfoItem ->
            if (contentAvailability in
                arrayOf(
...
fun ChannelInfo.toChannel() =
    Channel(
        ...
        tabs =
            tabs
                .filterNot { it.contentFilters.contains(ChannelTabs.VIDEOS) }
                .map { ChannelTab(it.contentFilters.first().lowercase(), it.toTabDataString()) },
        subscriberCount = subscriberCount,
    )

fun PlaylistInfo.toPlaylist() =
    Playlist(
        name = name,
...
fun CommentsInfoItem.toComment() =
    Comment(
        author = uploaderName,
        commentId = commentId,
...
fun Page.toNextPageString() =
    JsonHelper.json.encodeToString(
        NextPage(url, id, ids, cookies, body?.let { Base64.encodeToString(it, Base64.DEFAULT) }),
    )

fun String.toPage(): Page =
    with(JsonHelper.json.decodeFromString<NextPage>(this)) {
...
fun ListLinkHandler.toTabDataString() =
    JsonHelper.json.encodeToString(
        TabData(originalUrl, url, id, contentFilters, sortFilter),
    )

fun String.toListLinkHandler() =
    with(JsonHelper.json.decodeFromString<TabData>(this)) {
...
class NewPipeMediaServiceRepository : MediaServiceRepository {
    ...
    private val trendingCategories =
        TrendingCategory.entries.associate {
            when (it) {
    ...
    override suspend fun getStreams(videoId: String): Streams =
        withContext(Dispatchers.IO) {
            ...
            Streams(
                ...
                isShort =
                    resp.isShortFormContent ||
                        (resp.videoStreams + resp.videoOnlyStreams)
                            .firstOrNull()
                            ?.let { it.height > it.width } ?: false,
            )
        }
    ...
    override suspend fun getDeArrowContent(videoId: String): DeArrowContent? =
        runCatching {
            ...
            value.copy(
                thumbnails =
                    value.thumbnails.map { thumbnail ->
    ...
    private suspend fun getLatestVideos(channelInfo: ChannelInfo): Pair<List<StreamItem>, String?> {
        val relatedTab =
            channelInfo.tabs.find { it.contentFilters.contains(ChannelTabs.VIDEOS) }
