...
open class OnlinePlayerService : AbstractPlayerService() {
    ...
    private fun setStreamSource() {
        ...
        when (selectStreamSource(
            isLive = streams.isLive,
            hasVideoStreams = streams.videoStreams.isNotEmpty(),
            sabrAvailable = streams.serverAbrStreamingUrl != null && streams.videoPlaybackUstreamerConfig != null,
            ...
            StreamSourceType.SABR -> {
                ...
                val mediaItem = createMediaItem(
                    streams.serverAbrStreamingUrl.orEmpty().toUri(),
            ...
            StreamSourceType.DASH -> {
                // use the YT DASH manifest for livestreams, the generated one otherwise
                val dashUri =
                    if (streams.isLive) {
                        ProxyHelper.rewriteUrlUsingProxyPreference(
                            streams.dash.orEmpty()
                        ).toUri()
                    } else {
            ...
            StreamSourceType.HLS -> {
                ...
                val mediaItem = createMediaItem(
                    ProxyHelper.rewriteUrlUsingProxyPreference(streams.hls.orEmpty()).toUri(),
...
internal enum class StreamSourceType { SABR, DASH, HLS, NONE }

/**
 * Selects which stream source type to play.
 *
 * For livestreams a blank/empty [Streams.dash] is treated as absent. An empty
 * DASH URL previously produced an empty [Uri] that made Media3 fall back to the
 * file data source and crash with ENOENT, even when a valid HLS URL was
 * available; those livestreams now fall back to HLS.
 */
internal fun selectStreamSource(
    isLive: Boolean,
    hasVideoStreams: Boolean,
    sabrAvailable: Boolean,
    dashUrl: String?,
    hlsUrl: String?
): StreamSourceType = when {
    // SABR only supports non-livestreams
    !isLive && sabrAvailable -> StreamSourceType.SABR
    // use the YT DASH manifest for livestreams only when a DASH URL actually exists
    hasVideoStreams && (!isLive || !dashUrl.isNullOrBlank()) -> StreamSourceType.DASH
    // HLS as last fallback
    !hlsUrl.isNullOrBlank() -> StreamSourceType.HLS
    else -> StreamSourceType.NONE
}
