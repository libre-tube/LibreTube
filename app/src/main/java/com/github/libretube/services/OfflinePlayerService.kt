        ...
        override fun onPlaybackStateChanged(playbackState: Int) {
            ...
            if (playbackState == Player.STATE_READY && PlayerHelper.watchHistoryEnabled) {
                scope.launch(Dispatchers.IO) {
                    val video = downloadWithItems?.download?.toStreamItem() ?: return@launch
                    runCatching {
                        UserDataRepositoryHelper.userDataRepository.addToWatchHistory(video.toWatchHistoryEntry(exoPlayer?.currentPosition))
                    }
                }
            }
        }
