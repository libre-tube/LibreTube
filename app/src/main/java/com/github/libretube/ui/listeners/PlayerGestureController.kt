        ...
        override fun onScroll(
            ...
            when {
                e1.x < width * LEFT_AREA_VIEW_PERCENTAGE -> {
                    if (isFullscreen) listener.onSwipeLeftScreen(distanceY, e2)
                }

                e1.x > width * RIGHT_AREA_VIEW_PERCENTAGE -> {
                    if (isFullscreen) listener.onSwipeRightScreen(distanceY, e2)
                }

                else -> listener.onSwipeCenterScreen(distanceY, e2)
            }
