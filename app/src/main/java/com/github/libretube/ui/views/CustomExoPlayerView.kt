...
abstract class CustomExoPlayerView(
    ...
    override fun onSwipeLeftScreen(distanceY: Float, e2: MotionEvent) {
        if (!PlayerHelper.swipeGestureEnabled) {
            if (PlayerHelper.fullscreenGesturesEnabled) onSwipeCenterScreen(distanceY, e2)
            return
        }
    ...
    override fun onSwipeRightScreen(distanceY: Float, e2: MotionEvent) {
        if (!PlayerHelper.swipeGestureEnabled) {
            if (PlayerHelper.fullscreenGesturesEnabled) onSwipeCenterScreen(distanceY, e2)
            return
        }
    ...
    override fun onSwipeCenterScreen(distanceY: Float, e2: MotionEvent) {
        if (!PlayerHelper.fullscreenGesturesEnabled) return
        fullscreenGestureAnimationController.onSwipe(distanceY, e2)
    }
