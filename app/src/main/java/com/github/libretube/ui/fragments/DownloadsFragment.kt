...
class DownloadsFragmentPage : DynamicLayoutManagerFragment(R.layout.fragment_download_content) {
    ...
    override fun onViewCreated(
        ...
        lifecycleScope.launch(Dispatchers.Main) {
            val playlistItems =
                downloadPlaylistId?.let { playlistId ->
                    val playlist =
            ...
            val downloads =
                withContext(Dispatchers.IO) {
                    ...
                    if (downloadTab != DownloadTab.PLAYLIST) {
                        ...
                        downloads.filter { playlistItems.orEmpty().contains(it.download.videoId) }
                    }
                }
        ...
        view?.binding?.apply {
            when (status) {
                ...
                is DownloadStatus.Progress -> {
                    ...
                    val progressInfo =
                        progressBar.progress.formatAsFileSize() +
...
class PlaylistDownloadsFragmentPage : Fragment(R.layout.fragment_download_content) {
    ...
    override fun onViewCreated(
        ...
        val adapter =
            DownloadPlaylistAdapter { playlist ->
