...
object PlayerHelper {
    ...
    fun getDefaultResolution(
        ...
        var prefKey =
            if (NetworkHelper.isNetworkMetered(context)) {
    ...
    fun getIntentActionName(context: Context): String = context.packageName + "." + ACTION_MEDIA_CONTROL
    ...
    private fun getRemoteAction(
        ...
        val intent =
            Intent(getIntentActionName(activity))
    ...
    fun getCurrentPlayedCaptionFormat(player: Player): Format? = getCurrentFormatByTrackType(player, C.TRACK_TYPE_TEXT)

    fun getCurrentVideoFormat(player: Player): Format? = getCurrentFormatByTrackType(player, C.TRACK_TYPE_VIDEO)
    ...
    fun getAudioTrackNameFromFormat(
        ...
        return context
            .getString(R.string.audio_track_format)
            .format(
                if (audioLanguage == null) {
                    context.getString(R.string.unknown_audio_language)
                } else {
                    Locale
                        .forLanguageTag(audioLanguage)
    ...
    fun haveAudioTrackRoleFlagSet(
        ...
        isFlagSet(roleFlags, C.ROLE_FLAG_DESCRIBES_VIDEO) ||
    ...
    fun getPlayPauseActionIcon(player: Player) =
        when {
