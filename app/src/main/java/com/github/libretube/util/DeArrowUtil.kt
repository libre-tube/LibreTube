...
object DeArrowUtil {
    private fun extractTitleAndThumbnail(content: DeArrowContent): Pair<String?, String?> {
        return content.titles.firstOrNull { it.votes >= 0 || it.locked }?.title to
                content.thumbnails.firstOrNull {
                    it.thumbnail != null && !it.original && (it.votes >= 0 || it.locked)
                }?.thumbnail
    }

    private suspend fun fetchDeArrowContentBatch(videoIds: List<String>): Map<String, DeArrowContent>? {
    ...
    private suspend fun fetchDeArrowContentParallel(
        videoIds: List<String>
    ): Map<String, DeArrowContent> = coroutineScope {
        videoIds.chunked(50)
            .map { batch -> async { fetchDeArrowContentBatch(batch) ?: emptyMap() } }
            .awaitAll()
    ...
    suspend fun deArrowStreams(streams: Streams, vidId: String): Streams {
        ...
        val response = fetchDeArrowContentParallel(videoIds)
    ...
    suspend fun deArrowContentItems(contentItems: List<ContentItem>): List<ContentItem> {
        ...
        val videoIds = contentItems.asSequence()
        ...
        coroutineScope {
            contentItems.forEach { contentItem ->
                if (contentItem.type != "stream") return@forEach
