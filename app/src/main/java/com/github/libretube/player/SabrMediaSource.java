...
import com.github.libretube.player.PlayerEmsgHandler.PlayerEmsgCallback;
import com.github.libretube.player.manifest.AdaptationSet;
import com.github.libretube.player.manifest.SabrManifest;
import com.github.libretube.player.manifest.SabrManifestParser;
import com.github.libretube.player.manifest.Period;
import com.github.libretube.player.manifest.Representation;
import com.github.libretube.player.manifest.UtcTimingElement;
...
public final class SabrMediaSource extends BaseMediaSource {
  ...
  /** Factory for {@link SabrMediaSource}s. */
  ...
  public static final class Factory implements MediaSourceFactory {
    ...
    @Nullable private ParsingLoadable.Parser<? extends SabrManifest> manifestParser;
     ...
     *
     * <ul>
     *   <li>{@link DefaultSabrChunkSource.Factory}
    ...
    public Factory(DataSource.Factory dataSourceFactory) {
      this(new DefaultSabrChunkSource.Factory(dataSourceFactory), dataSourceFactory);
    }
     ...
     *     #createMediaSource(SabrManifest, MediaItem)}.
     */
    public Factory(
    ...
    public Factory setManifestParser(
        @Nullable ParsingLoadable.Parser<? extends SabrManifest> manifestParser) {
    ...
    public Factory setCompositeSequenceableLoaderFactory(
      ...
      this.compositeSequenceableLoaderFactory =
          checkNotNull(
              ...
              "SabrMediaSource.Factory#setCompositeSequenceableLoaderFactory no longer handles null"
     ...
     * sideloaded manifest.
     *
     * @param manifest The manifest. {@link SabrManifest#dynamic} must be false.
     * @return The new {@link SabrMediaSource}.
     * @throws IllegalArgumentException If {@link SabrManifest#dynamic} is true.
     */
     ...
     * @throws IllegalArgumentException If {@link SabrManifest#dynamic} is true.
     */
    public SabrMediaSource createMediaSource(SabrManifest manifest, MediaItem mediaItem) {
    ...
    /**
     * Returns a new {@link SabrMediaSource} using the current parameters.
     *
    ...
    public SabrMediaSource createMediaSource(MediaItem mediaItem) {
      ...
      @Nullable ParsingLoadable.Parser<? extends SabrManifest> manifestParser = this.manifestParser;
      if (manifestParser == null) {
        manifestParser = new SabrManifestParser();
      }
  ...
  public static final String DEFAULT_MEDIA_ID = "SabrMediaSource";
  ...
  private final ParsingLoadable.Parser<? extends SabrManifest> manifestParser;
  ...
  private SabrMediaSource(
      MediaItem mediaItem,
      @Nullable SabrManifest manifest,
      @Nullable DataSource.Factory manifestDataSourceFactory,
      @Nullable ParsingLoadable.Parser<? extends SabrManifest> manifestParser,
      SabrChunkSource.Factory chunkSourceFactory,
      CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory,
  ...
  protected void prepareSourceInternal(@Nullable TransferListener mediaTransferListener) {
    ...
    if (sideloadedManifest) {
      ...
      loader = new Loader("SabrMediaSource");
  ...
  public void releasePeriod(MediaPeriod mediaPeriod) {
    SabrMediaPeriod dashMediaPeriod = (SabrMediaPeriod) mediaPeriod;
  ...
  /* package */ void onSabrManifestRefreshRequested() {
    handler.removeCallbacks(simulateManifestRefreshRunnable);
    startLoadingManifest();
  }

  /* package */ void onSabrManifestPublishTimeExpired(long expiredManifestPublishTimeUs) {
  ...
  /* package */ void onManifestLoadStarted(
      ParsingLoadable<SabrManifest> loadable,
  ...
  /* package */ void onManifestLoadCompleted(
      ParsingLoadable<SabrManifest> loadable, long elapsedRealtimeMs, long loadDurationMs) {
    LoadEventInfo loadEventInfo =
        new LoadEventInfo(
            loadable.loadTaskId,
            loadable.dataSpec,
    ...
    SabrManifest newManifest = loadable.getResult();
    ...
    if (newManifest.dynamic) {
      ...
      if (isManifestStale) {
        if (staleManifestReloadAttempt++
          ...
          manifestFatalError = new SabrManifestStaleException();
        }
        return;
      }
      staleManifestReloadAttempt = 0;
      ...
      boolean useUriFromPreviousRequest =
          loadable.dataSpec.uri.equals(manifestUri)
              || (cmcdConfiguration != null
                  && removeFromUri(loadable.dataSpec.uri).equals(manifestUri));
  ...
  private static long getIntervalUntilNextManifestRefreshMs(
      SabrManifest manifest, long nowUnixTimeMs) {
    ...
    for (int i = 0; i < period.adaptationSets.size(); i++) {
      ...
      @Nullable SabrSegmentIndex index = representations.get(0).getIndex();
      if (index != null) {
  ...
  private static boolean isIndexExplicit(Period period) {
    for (int i = 0; i < period.adaptationSets.size(); i++) {
      @Nullable
      SabrSegmentIndex index = period.adaptationSets.get(i).representations.get(0).getIndex();
  ...
  private static final class SabrTimeline extends Timeline {
    ...
    private long getAdjustedWindowDefaultStartPositionUs(long defaultPositionProjectionUs) {
      ...
      com.github.libretube.player.manifest.Period period = manifest.getPeriod(periodIndex);
    ...
    private static boolean isMovingLiveWindow(SabrManifest manifest) {
  ...
  private final class DefaultPlayerEmsgCallback implements PlayerEmsgCallback {
    ...
    @Override
    public void onSabrManifestRefreshRequested() {
      SabrMediaSource.this.onSabrManifestRefreshRequested();
    }

    @Override
    public void onSabrManifestPublishTimeExpired(long expiredManifestPublishTimeUs) {
      SabrMediaSource.this.onSabrManifestPublishTimeExpired(expiredManifestPublishTimeUs);
    }
  }

  private final class ManifestCallback implements Loader.Callback<ParsingLoadable<SabrManifest>> {
    ...
    public void onLoadCanceled(
        ...
        long loadDurationMs,
        boolean released) {
      SabrMediaSource.this.onLoadCanceled(loadable, elapsedRealtimeMs, loadDurationMs);
    }
  ...
  @UnstableApi
  private Uri removeFromUri(Uri uri) {
    return (uri.isHierarchical() && uri.getQueryParameter(CmcdConfiguration.CMCD_QUERY_PARAMETER_KEY) != null) ?
