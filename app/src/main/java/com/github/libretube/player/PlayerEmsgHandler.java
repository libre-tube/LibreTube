 ...
 * "urn:mpeg:Sabr:event:2012", and value "1"/"2"/"3". When it encounters one of these messages, it
 * will handle the message according to Section 4.5.2.1 Sabr -IF IOP Version 4.1:
 *
 * <ul>
 ...
 * </ul>
 *
 * In both cases, the Sabr media source will be notified, and a manifest reload should be triggered.
 */
@UnstableApi
public final class PlayerEmsgHandler implements Handler.Callback {
  ...
  /** Callbacks for player emsg events encountered during Sabr live stream. */
  public interface PlayerEmsgCallback {

    /** Called when the current manifest should be refreshed. */
    void onSabrManifestRefreshRequested();
     ...
     */
    void onSabrManifestPublishTimeExpired(long expiredManifestPublishTimeUs);
  }
   ...
   *     messages that generate Sabr media source events.
  ...
  /* package */ boolean maybeRefreshManifestBeforeLoadingNextChunk(long presentationPositionUs) {
    ...
    if (manifestRefreshNeeded) {
      maybeNotifySabrManifestRefreshNeeded();
    }
  ...
  private void notifyManifestPublishTimeExpired() {
    playerEmsgCallback.onSabrManifestPublishTimeExpired(expiredManifestPublishTimeUs);
  }

  /** Requests Sabr media manifest to be refreshed if necessary. */
  private void maybeNotifySabrManifestRefreshNeeded() {
    ...
    playerEmsgCallback.onSabrManifestRefreshRequested();
  }
  ...
  /**
   * Returns whether an event with given schemeIdUri and value is a Sabr emsg event targeting the
   * player.
   */
  ...
  public final class PlayerTrackEmsgHandler implements TrackOutput {
    ...
    /**
     * For live streaming, check if the Sabr manifest is expired before the next segment start time.
     * If it is, the Sabr media source will be notified to refresh the manifest.
     *
