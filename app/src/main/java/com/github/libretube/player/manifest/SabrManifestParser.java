...
import com.github.libretube.player.manifest.SegmentBase.SegmentList;
import com.github.libretube.player.manifest.SegmentBase.SegmentTemplate;
import com.github.libretube.player.manifest.SegmentBase.SegmentTimelineElement;
import com.github.libretube.player.manifest.SegmentBase.SingleSegmentBase;
...
@UnstableApi
public class SabrManifestParser extends DefaultHandler
    implements ParsingLoadable.Parser<SabrManifest> {
  ...
  @Override
  public SabrManifest parse(Uri uri, InputStream inputStream) throws IOException {
    try {
  ...
  protected SabrManifest parseMediaPresentationDescription(XmlPullParser xpp, Uri documentBaseUri)
  ...
  protected SabrManifest buildMediaPresentationDescription(
  ...
  protected @C.SelectionFlags int parseSelectionFlagsFromRoleDescriptors(
    ...
    for (int i = 0; i < roleDescriptors.size(); i++) {
      ...
      if (Ascii.equalsIgnoreCase("urn:mpeg:dash:role:2011", descriptor.schemeIdUri)) {
        result |= parseSelectionFlagsFromSabrRoleScheme(descriptor.value);
      }
    }
    return result;
  }

  protected @C.SelectionFlags int parseSelectionFlagsFromSabrRoleScheme(@Nullable String value) {
    if (value == null) {
      return 0;
    }
    switch (value) {
  ...
  protected @C.RoleFlags int parseRoleFlagsFromRoleDescriptors(List<Descriptor> roleDescriptors) {
    ...
    for (int i = 0; i < roleDescriptors.size(); i++) {
      Descriptor descriptor = roleDescriptors.get(i);
      if (Ascii.equalsIgnoreCase("urn:mpeg:dash:role:2011", descriptor.schemeIdUri)) {
        result |= parseRoleFlagsFromSabrRoleScheme(descriptor.value);
      }
    }
    return result;
  }
  ...
  protected @C.RoleFlags int parseRoleFlagsFromSabrRoleScheme(@Nullable String value) {
    if (value == null) {
      return 0;
    }
    switch (value) {
